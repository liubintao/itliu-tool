...
/**
 * @Description: 关于SystemProperties的工具类
 * <p>
 * 1、统一风格的读取系统变量到各种数据类型，其中Boolean.readBoolean的风格不统计，Double则不支持，都进行了扩展.
 * <p>
 * 2、简单的合并系统变量(-D)，环境变量和默认值，已系统变量优先，在未引入commons config时使用.
 * <p>
 * 3、Properties本质上是一个HashTable，每次读写都会加锁，所以不支持频繁的System.getProperty(name)来检查系统内容变化，
 * 因此扩展了一个ListenableProperties,在其所关心的属性变化时进行通知.
 * <p>
 * @Author: robust
 * @CreateDate: 2019/7/24 10:09
 * @Version: 1.0
 */
public class SystemPropertiesUtil {

    /**
     * 读取Boolean类型的系统变量，为空时返回null，代表未设置，而不是{@link Boolean#getBoolean(String)}的false.
     *
     * @param name
     * @return
     */
    public static Boolean getBoolean(String name) {
        ...
        return BooleanUtils.toBooleanObject(ret);
    }
    ...
    public static synchronized void registerSystemPropertiesListener(PropertiesListener listener) {
        ...
        //将System的properties替换为ListenableProperties
    ...
    public static class ListenableProperties extends Properties {
        private transient List<PropertiesListener> listeners = new CopyOnWriteArrayList<>();
